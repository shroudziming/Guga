...
新生成的 `guga_reflection` 只允许 `appraisal` 和 `felt_response` 两个字段。已有记忆中
可能存在的 `relational_intent` 和 `interpretation_confidence` 保持原样用于历史审计，
迁移过程不批量改写旧事件；新建或更新的 reflection 不再生成这两个字段。
...
- 新生成的 `guga_reflection` 字段集合严格等于 `appraisal` 和 `felt_response`，两者均为
  非空字符串。
- 旧事件中的额外 reflection 字段不会触发批量数据迁移或读取失败。
- 不新增模型调用。
...
除确定性单元测试外，增加显式开启的真实 API 集成测试。该测试使用项目现有模型配置
和真实 `generate_structured_reply` / `generate_reply` 路径，不使用伪模型或固定返回值。

真实 API 测试默认跳过，只有设置 `GUGA_RUN_LIVE_API_TESTS=1` 时才执行，避免普通测试
意外产生费用或依赖网络。测试输入使用固定时间和明确事件，例如用户陈述一个带日期的
计划，使模型必须覆盖事件字段和 reflection 字段。

测试重点是字段契约，而不是只断言请求成功：

- 至少生成一个可验证的 semantic event operation。
- `operation`、`event_kind`、`subject`、`entity`、`description`、`time_expression`、
  `start_at`、`end_at`、`end_unknown`、`source_message_ids` 和 `confidence` 均按 schema
  出现并通过现有 validator。
- `guga_reflection` 必须存在，字段集合严格等于 `appraisal` 和 `felt_response`，且两个
  字段均为非空字符串。
- reflection 内容能体现完整 Skill 的朋友视角，但不得出现在客观 `description`、时间、
  状态或来源字段中。
- 捕获并保存脱敏后的结构化结果和字段覆盖报告，失败时能够区分 API、JSON、schema 和
  人格边界问题。
