...
- **正式语义检索**：BGE-M3 生成 1024 维归一化向量，FAISS `IndexFlatIP` 执行检索。
...
- semantic route 与现有词法、时间路由合并，最终上下文按以下优先级组织：
...
- `--no-semantic`：关闭语义索引，用于不加载 BGE-M3 的轻量流程测试。
...
RAG 的词法、BM25、BGE-M3 对照探针位于 `tools/rag_diagnostics/`。
