...
class PersonaManager:
    ...
    def load(self, persona_name: str) -> Persona:
        ...
        skill_path = str(data.get("skill_path", "")).strip()
        if skill_path:
            skill = load_persona_skill(self.personas_dir / skill_path, self.personas_dir)
            return Persona(
                name=data["name"],
                system_prompt=skill.body,
                description=data.get("description", ""),
                agent_id=str(data.get("agent_id", "")).strip() or data["name"],
                reflection_context=skill.body,
                source_path=skill.source_path,
                persona_fingerprint=skill.fingerprint,
                expression_tags=skill.expression_tags,
            )
